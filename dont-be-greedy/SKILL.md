---
name: dont-be-greedy
description: When a user uploads or references a data file (CSV, JSON, XLSX, TXT, LOG) or any file larger than 100KB, immediately estimate token cost. If over 30k tokens, chunk and summarize. Use when large files are detected to prevent context overflow and maintain agent performance.
---
...
**Workflow:**
1. Run `scripts/estimate_size.py sales.csv` - Output: 50MB, ~13M tokens
2. Way over 30k tokens. Run `scripts/chunker.py sales.csv`
...
**Workflow:**
1. Run `scripts/estimate_size.py config.json` - Output: 2KB, ~512 tokens
...
**Workflow:**
1. Run `scripts/estimate_size.py application.log` - Output: 500KB, ~128K tokens
